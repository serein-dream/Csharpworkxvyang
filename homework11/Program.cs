...
namespace homework6
{
    public class OrderDetails : Object
    {
        ...
        public OrderDetails(string spname, int spmoney, int spnumber)
        {
            ...
            this.goodId = Guid.NewGuid().ToString();
        }
        public OrderDetails()
        {
        ...
        public override int GetHashCode()
        {
            ...
            hashCode = hashCode * -1521134295 + Spnumber.GetHashCode();
            hashCode = hashCode * -1521134295 + EqualityComparer<string>.Default.GetHashCode(Spname);
            hashCode = hashCode * -1521134295 + Spmoney.GetHashCode();
            return hashCode;
        }
    }
    public class Order : Object
    {
        ...
        public List<OrderDetails> Sp { get; set; }
        ...
        public Order()
        {
            ...
            Sp = new List<OrderDetails>();
        }
        public Order(string customer, string id, List<OrderDetails> p)
        {
        ...
        public void AddItem(OrderDetails orderItem)
        {
            ...
            /*
            if (Sp.Contains(orderItem))
            {
                throw new ApplicationException($"orderDetails-{orderItem} is already existed!");
            }
            Sp.Add(orderItem);*/
        }
        public OrderDetails querybySpname(string thename)
        {
    ...
    public class OrderService
    {
        ...
        public static void Deleteorder(string orderId)
        {
            using (var db = new OrderContext())
            {
                var order = db.Orders.Include("Items").Where(o => Convert.ToInt32 (o.orderId) ==Convert.ToInt32( orderId)).FirstOrDefault();
        ...
        //查
        public static Order QueryById(string checkId)
        {
            return  Myorder.Where(m => m.orderId == checkId).FirstOrDefault();
        ...
        public static void EditOrder(Order order)
        {
            ...
            var thisorder = Myorder.Where(m => m.orderId == order.orderId).FirstOrDefault();
            if (thisorder == null)
            {
                throw new InvalidOperationException("要编辑的订单不存在！");
            }
            if (order.Sp != null && order.Sp.Count > 0)
            {
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Order Tobeload = new Order("王五", "6112", new List<OrderDetails> { new OrderDetails("挑战程序设计竞赛", 54, 1), new OrderDetails("计算机组成与设计", 130, 2) });
